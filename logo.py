...
class Logo(object):
    ...
    * VW12 for a word (used for analog values)
    ...
    def readInputs(self, vm_address):
        """
        Reads from I addresses of Siemens Logo. Examples: read("I10.2") 
        
        :param vm_address: of Logo memory (e.g. I30.1, I24)
        :returns: integer
        """
        area = snap7types.S7AreaPE
        db_number = 1
        size = 1
        start = 0
        wordlen = 0
        logger.debug("read, vm_address:%s" % (vm_address))
        if re.match("I[0-9]{1,4}\.[0-7]{1}", vm_address):
            ## bit value
            logger.info("read, Bit address: " + vm_address)
            address = vm_address[1:].split(".")
            # transform string to int
            ...
            start = (address_byte*8)+address_bit
            wordlen = snap7types.S7WLBit
        elif re.match("I[0-9]+", vm_address):
            ## byte value
            logger.info("Byte address: " + vm_address)
            start = int(vm_address[1:])
            wordlen = snap7types.S7WLByte
    ...
    def readOutputs(self, vm_address):
        """
        Reads from Q addresses of Siemens Logo. Examples: read("Q40")/ read("Q10.2") 
        
        :param vm_address: of Logo memory (e.g. Q30.1, Q24)
        :returns: integer
        """
        area = snap7types.S7AreaPA
        db_number = 1
        size = 1
        start = 0
        wordlen = 0
        logger.debug("read, vm_address:%s" % (vm_address))
        if re.match("Q[0-9]{1,4}\.[0-7]{1}", vm_address):
            ## bit value
            logger.info("read, Bit address: " + vm_address)
            address = vm_address[1:].split(".")
            # transform string to int
            ...
            start = (address_byte*8)+address_bit
            wordlen = snap7types.S7WLBit
        elif re.match("Q[0-9]+", vm_address):
            ## byte value
            logger.info("Byte address: " + vm_address)
            start = int(vm_address[1:])
            wordlen = snap7types.S7WLByte
    ...
    def readMemo(self, vm_address):
        """
        Reads from M addresses of Siemens Logo. Examples: read("M40")/ read("M10.2") 
        
        :param vm_address: of Logo memory (e.g. M30.1, M24)
        :returns: integer
        """
        area = snap7types.S7AreaMK
        db_number = 1
        size = 1
        start = 0
        wordlen = 0
        logger.debug("read, vm_address:%s" % (vm_address))
        if re.match("M[0-9]{1,4}\.[0-7]{1}", vm_address):
            ## bit value
            logger.info("read, Bit address: " + vm_address)
            address = vm_address[1:].split(".")
            # transform string to int
            ...
            start = (address_byte*8)+address_bit
            wordlen = snap7types.S7WLBit
        elif re.match("M[0-9]+", vm_address):
            ## byte value
            logger.info("Byte address: " + vm_address)
            start = int(vm_address[1:])
            wordlen = snap7types.S7WLByte
    ...
    def writeInputs (self, vm_address, value):
        """
        Writes to VM addresses of Siemens Logo.
        Example: write("I10", 200) or write("I10.3", 1)
    ...
    def writeOutputs (self, vm_address, value):
        """
        Writes to Q addresses of Siemens Logo.
        Example: write("Q10", 200) or write("Q10.3", 1)
    ...
    def writeMemo (self, vm_address, value):
        """
        Writes to M addresses of Siemens Logo.
        Example: write("M10", 200) or write("M10.3", 1)
